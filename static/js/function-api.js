...
async function cargarProductosDesdeAPI() {
  try {
    ...
    console.log(`Cargados ${productos.length} productos desde la API`);
  } catch (error) {
    console.error("Error:", error);
    alert("No se pudo conectar con el servidor. Verifique que el servidor esté ejecutándose.");
  }
}
...
function buscarProductoPorCodigo(evento) {
  if (evento.key === "Enter") {
    ...
    if (codigoCompleto.length > 0) {
      ...
      // Asegurar que el código tenga el formato correcto (3 dígitos con ceros a la izquierda)
...
function agregarProductoATabla(producto, cantidad = 1) {
  ...
  celdaPrecio.innerHTML = `$${producto[2].toFixed(2)}`;
...
async function finalizarVenta() {
  ...
  contenido.querySelector("#btn-procesar").onclick = async function () {
    ...
    if (montoPagado >= totalProductos) {
      ...
      try {
        // Solo registra la venta con fecha y hora (según tu estructura de BD actual)
        const response = await fetch(`${API_URL}/ventas`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({}), // Tu API actual no requiere datos
        });
        ...
        if (response.ok) {
          ...
          alert(
            `Error al procesar la venta: ${errorData.error || 'Error desconocido'}`
          );
        }
      } catch (error) {
      ...
      // Limpiar la tabla después de procesar
      while (tabla.rows.length > 0) {
        tabla.deleteRow(0);
      }
      totalProductos = 0;
    ...
    try {
      ...
      productosFiltrados.forEach((producto) => {
        ...
        html += `<tr>
          ...
          <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>${producto.precio_producto.toFixed(2)}</td>
      ...
      productosFiltrados.forEach((producto) => {
        html += `<tr>
          ...
          <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>${producto[2].toFixed(2)}</td>
          <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>
...
async function mostrarInventario() {
  ...
  html +=
    "<tr style='background-color: #f2f2f2;'><th style='padding: 8px; border: 1px solid #ddd;'>Código</th><th style='padding: 8px; border: 1px solid #ddd;'>Producto</th><th style='padding: 8px; border: 1px solid #ddd;'>Precio</th><th style='padding: 8px; border: 1px solid #ddd;'>Existencias</th></tr>";
  ...
  try {
    ...
    if (response.ok) {
      ...
      productosAPI.forEach((producto) => {
        ...
        html += `<tr>
          ...
          <td style='padding: 8px; border: 1px solid #ddd;'>${producto.nombre_producto}</td>
          <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>${producto.precio_producto.toFixed(2)}</td>
          <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>${producto.existencias_producto || 0}</td>
        </tr>`;
      });
    }
  } catch (error) {
    productos.forEach((producto) => {
      html += `<tr>
        ...
        <td style='padding: 8px; border: 1px solid #ddd;'>${producto[1]}</td>
        <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>${producto[2].toFixed(2)}</td>
        <td style='padding: 8px; border: 1px solid #ddd; text-align: center;'>${producto[3] || 0}</td>
      </tr>`;
    });
  }
